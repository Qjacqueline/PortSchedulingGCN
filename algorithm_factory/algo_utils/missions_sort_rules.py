...
class sort_missions(object):
    ...
    @staticmethod
    def FCFS(missions):
        missions.sort(key=lambda x: x.total_process_time + x.machine_start_time[0])  # 按照抵达exit时间进行排序
    ...
    def RELEASE_ORDER(missions):
        missions.sort(key=lambda x: (x.machine_start_time[0], int(x.idx[1:])))  # 按照释放时间排序
    ...
    def A_STATION_NB(missions):
        missions.sort(
            key=lambda x: (
                x.machine_start_time[2], int(x.idx[1:])))  # 按照到达锁站时间排序
