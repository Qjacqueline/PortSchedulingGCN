...
from algorithm_factory.algo_utils.data_buffer import UANewBuffer, ua_new_collate_fn
...
from algorithm_factory.rl_algo.upper_agent_new import ACNew, UANewCollector, h_new_train
...
if __name__ == '__main__':
    ...
    # ========================= Policy ======================
    u_agent = ACNew(actor=ActorNew(args.m_max_num, args.dim_mach_fea, args.dim_yard_fea, args.hidden_size,
                                   args.n_layers, args.device),
                    critic=CriticNew(args.m_max_num, args.dim_mach_fea, args.dim_yard_fea, args.hidden_size,
                                     args.n_layers, args.device),
                    ...
                    device=args.device)
    l_agent = DDQN(
        eval_net=QNet(args.m_max_num, args.dim_mission_fea, args.dim_mach_fea, args.dim_yard_fea, args.hidden_size,
                      args.n_layers, args.device),
        target_net=QNet(args.m_max_num, args.dim_mission_fea, args.dim_mach_fea, args.dim_yard_fea, args.hidden_size,
                        args.n_layers, args.device),
    ...
    u_dl_train = DataLoader(dataset=data_buffer, batch_size=args.u_batch_size, collate_fn=ua_new_collate_fn,
                            num_workers=2)
    ...
    for i in range(100):
        u_collector.collect_rl()
        logger.info("开始第" + str(i) + "训练")
        h_new_train(epoch_num=1, u_dl_train=u_dl_train, u_agent=u_agent, l_agent=l_agent,
