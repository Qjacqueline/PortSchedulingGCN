...
if __name__ == '__main__':
    ...
    rs_flag = False
    for j in range(1):
        # m_num_ls = [i + 40 for i in range(10)]  # , 500, 1000
        # inst_type_ls = ['Z2' for _ in range(10)]  # chr(65 + j) len(m_num_ls)
        ...
        for i in range(len(m_num_ls)):
            ...
            lb2s.append(-1)
            lb3s.append(-1)
            # if model.MLP.status != 1 or rs_flag:
            #     ms.append(model.MLP.getVars()[-2].x)
            # else:
            ms.append(-1)
            f.write("instance\t" + inst_type_ls[0] + str(m_num_ls[0]) + "\t" + str(lb1) + "\n")
        # f.write(chr(65 + j) + "_LB\n")
        # for i in range(len(m_num_ls)):
        # f.write("算例为\t" + inst_type_ls[i] + str(m_num_ls[i]) +
        #         "\t松弛解为\t" + str(ms[i]) +
        #         "\tlb1为\t" + str(lb1s[i]) +
        #         "\tlb2为\t" + str(lb2s[i]) + "\tlb3为\t" + str(lb3s[i]) + '\tmin\t' + str(
        #     max(lb1s[i], lb3s[i])) + "\n")

        # f.write(str(max(lb1s[i], lb3s[i])) + "\n")
        # f.write("instance\t" + inst_type_ls[0] + str(m_num_ls[0]) + "\t" + str(sum(lb1s) / 10) + "\n")
    f.close()
